...
> **Last updated:** 2026-05-09 — reconciliation v1: directory naming (singular plural→singular per actual MSP layout), npm scripts (drop msp:check/codegen/compose, use msp:validate + msp:check-links + msp:run-task + msp:master), MCP tool surface §16.5 (full 11-tool table), bin entries (5 bins), Master Dashboard / 14_devlog / generic-boilerplate folders flagged as aspirational
> **Earlier:** 2026-05-09 — added Master Block (§3.6: 3-tier knowledge model + `msp:master compose` loader)
...
| **Blueprint** | YAML คำสั่งงานทางเทคนิค — ไร้น้ำ ไว้ให้ SLM อ่าน | `gks/blueprint/BLUEPRINT--FEAT-NNN.yaml` |
| **Micro-task** | 1 concern 1 ไฟล์ YAML — หน่วยย่อยสุดของ codegen | `gks/task/FEAT-NNN/T*.task.yaml` |
...
├── gks/                             # 🧠 The Shared Brain (singular folder names — MSP convention)
│   ├── 00_index/                    # L0 search index
...
│   ├── blueprint/                   # 🔴 (BLUEPRINT--*)
│   ├── audit/                       # 🟠 (AUDIT--* — phase 6)
│   ├── master/                      # 🟣 (MASTER--* — distilled Genesis, see §3.6)
│   ├── proto/                       # 🛡️ (PROTO--* — validator predicates)
│   ├── task/                        # (TASK--* — used rarely; codegen runs inline)
│   └── issues/                      # legacy — not actively used
├── .brain/msp/                      # 🛡️ Local runtime + governance (SSOT for contracts)
...
### 4.2.1 ประเภทความรู้: Ideas (aspirational — ไม่ได้ใช้ใน MSP)

> 🟡 **Status:** ในเวอร์ชันบอยเลอร์เพลตนี้กำหนดให้มี `ideas/` แต่ MSP repo จริง**ไม่ได้ใช้** — ไอเดียตั้งต้นถูกบันทึกตรงเป็น `CONCEPT--*` แทน

- **ID Prefix:** `IDEA--` (Phase 0)
- **เป้าหมาย:** เก็บ Raw Prompts และไอเดียตั้งต้น (The Spark)
- **ผู้ใช้หลัก:** Human / PM

### 4.3 ประเภทความรู้: Concepts (`concept/`)
...
|---|---|---|---|
| `CONCEPT--` | `concept/` | Concept | ความต้องการ + นิยามเชิงมนุษย์ (Phase 1) |
| `ADR--` | `adr/` | Architecture Decision Record | การตัดสินใจทางสถาปัตยกรรม (Phase 2) |
| `FEAT--` | `feat/` | Feature Spec | รายละเอียดฟีเจอร์และพฤติกรรมระบบ (Phase 2) |
| `FRAME--` | `frame/` | Framework Rules | สถาปัตยกรรม + มาตรฐานโค้ด (Phase 0) |
| `BLUEPRINT--`| `blueprint/`| Implementation Plan | YAML/MD แผนงานทางเทคนิค (Phase 3) |
| `AUDIT--` | `audit/` | Post-implementation Audit | สิ่งที่ ship แล้ว vs blueprint (Phase 6) |
| `MASTER--` | `master/` | **Master Block** (3-tier) | Genesis ที่ถูก promote เป็น absolute meaning — ดู §3.6 |
| `PROTO--` | `proto/` | Validator Predicate | กฎ runtime ที่ enforce ผ่าน `npx tsx src/validator/cli.ts --all` |
| `TASK--` | `task/` | Micro-task spec | (ใช้น้อย — codegen รันใน `src/codegen/` ตรง) |

> 🔵 **บอยเลอร์เพลตเดิม** มี `algorithms/`, `entities/`, `flows/`, `modules/`, `parameters/`, `ideas/`, `ops/` ซึ่งเป็นโครงสร้างทั่วไปสำหรับโปรเจกต์อื่น — MSP repo เลือกใช้แค่ subset ข้างบน ถ้า fork ไปใช้กับโปรเจกต์อื่น สามารถเพิ่มโฟลเดอร์เหล่านั้นได้ตามต้องการ
...
### 4.6 ประเภทความรู้: Micro-tasks (`task/`)
...
### 4.10 กลไกเชื่อมโยง — Master Dashboard (aspirational — ไม่มีในปัจจุบัน)

> 🟡 **Status:** บอยเลอร์เพลตเดิมเสนอ `gks/00_MASTER_DASHBOARD.md` เป็น live command center แต่ MSP repo จริง**ยังไม่มี** — แทนที่ด้วย:
> - Atomic index (`gks/00_index/atomic_index.jsonl`) สำหรับ machine consumption
> - `ROADMAP.md` (root) สำหรับ human-readable status

วิสัยทัศน์เดิม (ถ้า fork ไปใช้):
- **Global visibility:** สถานะของแต่ละ FEAT ข้ามทั้ง phase
- **Cross-vault linkage:** relative path หรือ absolute URI เชื่อม vault
- **Multi-vault integration:** เปิด vault ย่อย ได้เฉพาะเมื่อต้องการ
...
| `task/*` | ✅ ได้ (T2/T3) | runner validates at execution |
...
manifest:
  ...
  blueprint_ref: "gks/blueprint/BLUEPRINT--FEAT-001.yaml#api_contracts[0]"
...
### 8.6 Commands (MSP-specific)

```bash
npm run msp:run-task <FEAT-ID> <task-id>   # T1 รัน task เดียว (output ถูกตรวจ + เขียนตรงเข้า src/)
# หมายเหตุ: บอยเลอร์เพลตเดิมแยกเป็น msp:codegen + msp:compose (กระบวนการ 2 ขั้น);
# MSP รวมขั้นตอนใน src/codegen/runner.ts — ไม่มี deterministic-join ขั้นแยก
```
...
- ไฟล์ที่ผลิตจาก Phase 3.5 ต้องขึ้นต้นด้วย comment:
  ...
  // Source: gks/task/FEAT-NNN/manifest.yaml
  // Regenerate: npm run msp:run-task FEAT-NNN <task-id>
  ```
...
| `gks/<type>/*` (ทุก atomic — concept, adr, feat, frame, blueprint, audit, master, proto) | `msp_candidate` MCP tool → `.brain/.../candidates/` → human PR promotes — ห้ามเขียน `gks/<type>/` ตรงๆ (per `ADR--AGENT-WRITE-BOUNDARIES`) |
...
### 15.1 MSP / GKS commands (ของจริงใน `package.json`)

```bash
# Atom validation + indexing
npm run msp:index            # regen gks/00_index/atomic_index.jsonl
npm run msp:validate -- --all   # full validator + PROTO predicates (used in pre-commit)
npm run msp:check-links      # gks crosslink integrity scan
npm run msp:verify <FEAT-ID> # gks verify-flow per FEAT (used in pre-push)
npm run msp:backlinks        # rebuild .brain/.../vector/backlinks.jsonl

# Codegen (Phase 3.5)
npm run msp:run-task <FEAT-ID> <task-id>

# Master Block compose (3-tier — ดู §3.6)
npm run msp:master compose <MASTER-ID> [<MASTER-ID> ...]

# Hotfix workflow (per ADR--HOTFIX-ESCAPE-HATCH)
npm run msp:hotfix:open
npm run msp:hotfix:list
npm run msp:hotfix:close
npm run msp:hotfix:check
```

> 🔵 **Bin entries** (เปิดใช้หลัง `npm run build`): `msp-validate`, `msp-backlinks`, `msp-run-task`, `msp-master`, `msp-mcp-server` (ทั้ง 5 ตัว map ไปที่ compiled `dist/`).
...
```bash
#!/usr/bin/env bash
# จริงๆ MSP ใช้ examples/hooks/pre-commit.sh ที่รัน:
npm run msp:validate -- --all || {
  echo "MSP validator failed — see error output"
  exit 1
}
npm run msp:check-links || {
  echo "MSP crosslink resolution failed"
  exit 1
}
```
...
### 16.5 MCP tool surface (จริงใน MSP repo — `src/mcp/server.ts`)

11 ตัวลงทะเบียนใน `TOOLS` array; ทดสอบ exact list ที่ `test/mcp/server.test.ts:12`

| MCP tool | Module | Purpose |
|---|---|---|
| `msp_validate` | `tools/validate.ts` | Run validator (`--all` walks `gks/`) |
| `msp_candidate` | `tools/candidate.ts` | Draft atomic → `.brain/.../candidates/` (single intake path) |
| `msp_run_task` | `tools/run-task.ts` | T1 codegen — รัน task YAML ผ่าน `src/codegen/runner.ts` |
| `msp_session_append` | `tools/session-append.ts` | Append turn ไป session JSONL (M7-prep) |
| `msp_episode_append` | `tools/episode-append.ts` | Append episodic memory entry (M7b consolidator) |
| `msp_backlinks_rebuild` | `tools/backlinks-rebuild.ts` | Regen `.brain/.../vector/backlinks.jsonl` |
| `msp_recall` | `tools/recall.ts` | Retrieval orchestration (RRF fuse: Obsidian + episodic + vector + backlinks; M7c) |
| `msp_remember` | `tools/remember.ts` | Hint memory (importance signal for consolidator) |
| `msp_compress` | `tools/compress.ts` | Three-tier context compression (M7d) |
| `msp_identity_get` | `tools/identity-get.ts` | Read agent profile / soul (M7e) |
| `msp_identity_set` | `tools/identity-set.ts` | Update agent profile / soul (M7e) |

> Slash commands `/checkpoint` / `/catchup` / `/new-feature` / `/verify-flow` ที่บอยเลอร์เพลตเดิมเสนอ — MSP ยังไม่ implement; ใช้ MCP tools ตามตารางข้างบนแทน
...
| `gks/audit/AUDIT--*.md` | Post-implementation record — what shipped vs blueprint |
...
### Step 1 — Skeleton (MSP-actual layout)
```bash
# โครงโฟลเดอร์ที่ MSP ใช้จริง (singular names)
mkdir -p gks/{00_index,concept,adr,feat,frame,blueprint,audit,master,proto,task,issues}
mkdir -p .brain/msp/{LLM_Contract,projects} scripts/msp src test
touch gks/00_index/atomic_index.jsonl
touch CHANGELOG.md ROADMAP.md
```

> 🟡 บอยเลอร์เพลตเดิมรวม `ideas/`, `algorithms/`, `entities/`, `flows/`, `modules/`, `parameters/`, `ops/`, `14_devlog/` แต่ MSP repo จริงไม่ได้ใช้ — เพิ่มได้ตามต้องการเมื่อ fork ไปใช้กับโปรเจกต์อื่น
...
ติดตั้ง hook ตาม `examples/hooks/pre-commit.sh` ซึ่งรัน `npm run msp:validate -- --all` + `npm run msp:check-links`
...
- `gks/concept/CONCEPT--ExampleFeature.md` → review → APPROVED
- `gks/feat/FEAT--example-feature.md` (ผ่าน MSP)
- `gks/blueprint/BLUEPRINT--FEAT-001.yaml`
- code → `src/...`
...
*End of CORE_FRAMEWORK_MASTER_SPEC.md — ไฟล์นี้คือ baseline บริสุทธิ์ ห้ามเพิ่ม business logic ลงไป ถ้าต้องการเพิ่ม domain-specific ให้สร้าง atomic notes ในโฟลเดอร์ของ knowledge types (เช่น `gks/concept/`, `gks/feat/`, `gks/master/`) แทน*
