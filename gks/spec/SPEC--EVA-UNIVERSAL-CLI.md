...
The `eva` command is the global entry point for the entire ecosystem. It simplifies the setup and orchestration of Cognitive System projects by providing a single, memorable command for installation, bootstrapping, and agent invocation.

## 2. Command Surface

### 2.1 `eva init [dir]`
Installs the core framework into the specified directory.
-   **Action:** Runs the `bootstrap` logic (copying dist files, core axioms, and schemas).
-   **Result:** A clean project folder ready for implementation.

### 2.2 `eva vibe`
Launches the autonomous EVA-CLI agent in the current directory.
-   **Action:** Invokes the `msp-mll` or the upcoming autonomous implementation loop.
...
### 3.2 Global Shim
When installed via `npm install -g eva-cli`, the `eva` command is added to the system PATH.
