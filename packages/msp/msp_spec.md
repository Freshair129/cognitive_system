...
### 2.1b Web UI Trigger Flow (Covibe Backlog)

```
┌─────────────────────────────────────────────────────────────┐
│  Covibe Web UI (covibe_roadmap.html)                        │
│  - User clicks "Let Agent Do This" on a task                │
│  - Web UI extracts `data-task-id`                           │
└─────────────┬───────────────────────────────────────────────┘
              │ WebSocket Connection (ws://127.0.0.1:8787)
              ▼
┌─────────────────────────────────────────────────────────────┐
│  MSP WS Agent Server (src/agents/ws-agent-server.ts)        │
│  - Validates connection                                     │
│  - Resolves task ID via `atomic_index.jsonl`                │
└─────────────┬───────────────────────────────────────────────┘
              │ Spawn CLI Process (`qwen-cli` / `eva-cli`)
              ▼
┌─────────────────────────────────────────────────────────────┐
│  Agent runtime streams stdout/stderr back via WebSocket     │
│  Web UI updates terminal box in real-time                   │
└─────────────────────────────────────────────────────────────┘
```
```
